...
export class GraphQLCache implements GraphQLCacheInterface {
	...
	getFragmentDefinitions = async (projectConfig: GraphQLProjectConfig): Promise<Map<string, FragmentInfo>> => {
	...
	getObjectTypeDefinitions = async (projectConfig: GraphQLProjectConfig): Promise<Map<string, ObjectTypeInfo>> => {
		// This function may be called from other classes.
		// If then, check the cache first.
		const rootDir = projectConfig.dirpath;
	...
	async updateFragmentDefinition(rootDir: Uri, filePath: Uri, contents: Array<CachedContent>): Promise<void> {
	...
	async updateFragmentDefinitionCache(rootDir: Uri, filePath: Uri, exists: boolean): Promise<void> {
		const fileAndContent = exists ? await this.promiseToReadGraphQLFile(filePath) : null;
	...
	async updateObjectTypeDefinition(rootDir: Uri, filePath: Uri, contents: Array<CachedContent>): Promise<void> {
	...
	async updateObjectTypeDefinitionCache(rootDir: Uri, filePath: Uri, exists: boolean): Promise<void> {
		const fileAndContent = exists ? await this.promiseToReadGraphQLFile(filePath) : null;
	...
	getSchema = async (appName?: string, queryHasExtensions?: boolean | null): Promise<GraphQLSchema | null> => {
		...
		if (!schema && schemaPath && schemaKey) {
			...
			if (this._schemaMap.has(schemaCacheKey)) {
				...
				if (schema) {
					return queryHasExtensions ? this._extendSchema(schema, schemaPath, schemaCacheKey) : schema;
				}
			}
	...
	_invalidateSchemaCacheForProject(projectConfig: GraphQLProjectConfig) {
		const schemaKey = this._getSchemaCacheKeyForProject(projectConfig) as string;
		schemaKey && this._schemaMap.delete(schemaKey);
	}

	_getSchemaCacheKeyForProject(projectConfig: GraphQLProjectConfig): any {
