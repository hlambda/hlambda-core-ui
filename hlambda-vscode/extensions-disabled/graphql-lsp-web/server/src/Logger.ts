...
import { DIAGNOSTIC_SEVERITY, SeverityEnum, SEVERITY } from 'graphql-language-service';
...
export class Logger implements VSCodeLogger {
	...
	constructor(tmpDir?: string, stderrOnly?: boolean) {
		...
		const dir = join(tmpDir || '/tmp', 'graphql-language-service-logs');
		try {
		...
		this._logFilePath = join(dir, `graphql-language-service-log-${'webworker'}-${getDateString()}.log`);
