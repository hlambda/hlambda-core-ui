...
import { parseDocument, DEFAULT_SUPPORTED_EXTENSIONS, DEFAULT_SUPPORTED_GRAPHQL_EXTENSIONS } from './parseDocument';
...
const buildOptions = (options: ServerOptions): MappedServerOptions => {
	...
	if (serverOptions.loadConfigOptions) {
		...
		if (serverOptions.extensions) {
			serverOptions.loadConfigOptions.extensions = [...serverOptions.extensions, ...(extensions || [])];
		}
	} else {
 ...
 */
export default async function startServer(options: ServerOptions): Promise<void> {
	if (options?.method) {
...
async function initializeHandlers({ reader, writer, logger, options }: InitializerParams): Promise<Connection> {
	try {
...
function reportDiagnostics(diagnostics: PublishDiagnosticsParams | null, connection: Connection) {
	if (diagnostics) {
...
async function addHandlers({
	...
	connection.onRequest(InitializeRequest.type, (params, token) =>
		messageProcessor.handleInitializeRequest(params, token, loadConfigOptions.rootDir)
	);
