...
import { Hover, SymbolInformation, SymbolKind } from 'vscode-languageserver-types';
...
export class GraphQLLanguageService {
	...
	public async getDiagnostics(document: string, uri: Uri, isRelayCompatMode?: boolean): Promise<Array<Diagnostic>> {
		...
		try {
			...
			if (error instanceof GraphQLError) {
				const range = getRange(error.locations?.[0] ?? { column: 0, line: 0 }, document);
				return [
					{
		...
		return validateQuery(validationAst, schema, customRules as ValidationRule[], isRelayCompatMode);
	}
	...
	public async getAutocompleteSuggestions(
		...
		if (schema) {
			return getAutocompleteSuggestions(schema, query, position, undefined, fragmentInfo, { uri: filePath });
		}
		return [];
	}
	...
	public async getDefinition(query: string, position: IPosition, filePath: Uri): Promise<DefinitionQueryResult | null> {
		const projectConfig = this.getConfigForURI(filePath);
		if (!projectConfig) {
			return null;
		}
	...
	public async getDocumentSymbols(document: string, filePath: Uri): Promise<SymbolInformation[]> {
	...
	async _getDefinitionForNamedType(
		...
		const typeCastedDefs = localObjectTypeDefinitions as any as Array<TypeDefinitionNode>;

		const localOperationDefinitionInfos = typeCastedDefs.map((definition: TypeDefinitionNode) => ({
			filePath,
			content: query,
			definition,
		}));
	...
	async _getDefinitionForFragmentSpread(
		...
		const typeCastedDefs = localFragDefinitions as any as Array<FragmentDefinitionNode>;

		const localFragInfos = typeCastedDefs.map((definition: FragmentDefinitionNode) => ({
			filePath,
			content: query,
			definition,
		}));
