...
export class MessageProcessor {
	...
	constructor({
		...
		if (loadConfigOptions.extensions && loadConfigOptions.extensions?.length > 0) {
	...
	async handleInitializeRequest(
		...
		this._rootPath = configDir ? configDir.trim() : params.rootUri || this._rootPath;
		if (!this._rootPath) {
	...
	async _updateGraphQLConfig() {
		...
		try {
			...
			if (this._graphQLConfig || this._graphQLCache?.getGraphQLConfig) {
				const config = this._graphQLConfig ?? this._graphQLCache.getGraphQLConfig();
	...
	async handleDidOpenOrSaveNotification(
		...
		if ('text' in textDocument && textDocument.text) {
			...
			const hasGraphQLConfigFile = configMatchers.some((v) => uri.match(v)?.length);
			const hasPackageGraphQLConfig = uri.match('package.json')?.length;
		...
		if (!this._graphQLCache) {
			...
			try {
				...
				if (this._isInitialized && project?.extensions?.languageService?.enableValidation !== false) {
					await Promise.all(
	...
	async handleDidChangeNotification(params: DidChangeTextDocumentParams): Promise<PublishDiagnosticsParams | null> {
		if (this._isGraphQLConfigMissing || !this._isInitialized || !this._graphQLCache) {
			return null;
		}
	...
	async handleCompletionRequest(params: CompletionParams): Promise<CompletionList | Array<CompletionItem>> {
		if (!this._isInitialized || !this._graphQLCache) {
			return [];
		}
		...
		return Promise.all(
			params.changes.map(async (change: FileEvent) => {
				if (this._isGraphQLConfigMissing || !this._isInitialized || !this._graphQLCache) {
					...
					return;
				} else if (change.type === FileChangeTypeKind.Created || change.type === FileChangeTypeKind.Changed) {
	...
	async handleDocumentSymbolRequest(params: DocumentSymbolParams): Promise<Array<SymbolInformation>> {
		if (!this._isInitialized || !this._graphQLCache) {
			return [];
		}
	...
	async handleWorkspaceSymbolRequest(params: WorkspaceSymbolParams): Promise<Array<SymbolInformation>> {
		if (!this._isInitialized || !this._graphQLCache) {
			return [];
		}
		...
		if (params.query !== '') {
			...
			return symbols.filter((symbol) => symbol?.name && symbol.name.includes(params.query));
		}
	...
	async _cacheSchemaFile(uri: any, project: GraphQLProjectConfig) {
		uri = uri.toString();
	...
	_getTmpProjectPath(project: GraphQLProjectConfig, prependWithProtocol = true, appendPath?: string) {
	...
	async _cacheSchemaPath(uri: string, project: GraphQLProjectConfig) {
		console.log("CAN'T USE GLOB");
		// try {
		// 	const files = await glob(uri);
		// 	if (files && files.length > 0) {
		// 		await Promise.all(files.map((uriPath: UnnormalizedTypeDefPointer) => this._cacheSchemaFile(uriPath, project)));
		// 	} else {
		// 		try {
	...
	async _cacheObjectSchema(pointer: { [key: string]: any }, project: GraphQLProjectConfig) {
		await Promise.all(Object.keys(pointer).map(async (schemaUri) => this._cacheSchemaPath(schemaUri, project)));
	}
	async _cacheArraySchema(pointers: any[], project: GraphQLProjectConfig) {
		await Promise.all(
	...
	async _cacheSchemaFilesForProject(project: GraphQLProjectConfig) {
		...
		const cacheSchemaFileForLookup =
			config?.cacheSchemaFileForLookup ?? this?._settings?.cacheSchemaFileForLookup ?? false;
	...
	async _updateFragmentDefinition(uri: Uri, contents: CachedContent[]): Promise<void> {
	...
	async _updateObjectTypeDefinition(uri: Uri, contents: CachedContent[]): Promise<void> {
		const rootDir = this._graphQLCache.getGraphQLConfig().dirpath;
	...
	async _invalidateCache(
		...
		if (this._textDocumentCache.has(uri)) {
			const cachedDocument = this._textDocumentCache.get(uri);
			if (cachedDocument && textDocument && textDocument?.version && cachedDocument.version < textDocument.version) {
...
function processDiagnosticsMessage(results: Diagnostic[], query: string, range: RangeType | null): Diagnostic[] {
	...
	if (range) {
		...
		return processedResults.map((diagnostic) => ({
			...diagnostic,
			range: new Range(
				new Position(diagnostic.range.start.line + offset.line, diagnostic.range.start.character),
				new Position(diagnostic.range.end.line + offset.line, diagnostic.range.end.character)
			),
		}));
	}
